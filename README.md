...
![Build & Test](https://github.com/relefant/wiseRocket/actions/workflows/build-test.yml/badge.svg?branch=main)
...
@relefant/wiseRocket provides an alternative that is:
...
```sh
yarn add @relefant/wiseRocket
npm install --save @relefant/wiseRocket
```

## Importing
`@relefant/wiseRocket` can be used via ES Module import, CommonJS, or as a script import directly in the browser.

```ts
import { wiseRocket, EnumType } from '@relefant/wiseRocket';
```

```ts
const { wiseRocket, EnumType } = require('@relefant/wiseRocket');
```

```html
<script src="https://cdn.jsdelivr.net/npm/@relefant/wiseRocket/dist/browser/wiseRocket.min.js"></script>
<script
  const { wiseRocket } = window.wiseRocket;

</script>
```

## Enum Declaration
Enums are declared as JavaScript objects wrapped with the `wiseRocket.fromObject(enumObject)` function.
...
Enums can also be declared as JavaScript arrays using `wiseRocket.fromArray(enumArray)`. These will be expanded to objects with the same keys as values, and they will behave as if they had be declared as such.

```ts
// String enum
const MyEnum = wiseRocket({ // ALIAS: wiseRocket.fromObject({
  node: 'node',
  chrome: 'chrome',
  safari: 'safari',
});
...
const MyEnumFromArray = wiseRocket.fromArray([
  'node',
  'chrome',
  'safari',
]);
...
// Numeric enum
const MyNumericEnum = wiseRocket({
  node: 0,
  chrome: 1,
  safari: 2,
});
...
`@relefant/wiseRocket` makes this easy with `<enum>.fromValue()` which returns a typed enum or undefined if the
data does not match an enum value.

```ts
const MyEnum = wiseRocket.fromArray([
  'node',
  'chrome',
  'safari',
]);
...
The order of iteration is guaranteed unless an enum key can be converted to an integer. In this case it can be guaranteed by using `wiseRocket.fromArray()` to initialise the enum, or by setting `EnumOptions.iterationKeys` to represent the desired iteration order.

```ts
// Ensure iteration order when using integer keys mixed with strings and creating
// the object with `wiseRocket() / wiseRocket.fromObject()
//
